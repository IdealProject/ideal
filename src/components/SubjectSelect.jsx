        ...
        <div className="flex py-4 px-4">
            ...
            <section className="w-3/4 p-4 m-4 bg-slate-200 border-5px boder-black rounded-lg ">
                ...
                {majorData[0].value == 'cpa' ? // en el caso de ser el cpa 
                    getMajores(majorData[0].value).map(e => <li><a href={`${majorData[0].value}-${e.value}`} >{e.label}</a></li>) // mostramos sus materias unicamente
                    :
                    //  en el caso contrario mostramos el selector de semestres y materias para cada uno
                    getMajores(majorData[0].value)[semester].map(e => <li><a href={`${majorData[0].value}-${e.value}`}>{e.label}</a></li>)}
            </section>
        </div>
    );
};
